...
const JbSeekDashBoard = () => {
	...
	useEffect(() => {
		axios
		  ...
		  .then((res) => {
			setname(res.data[0].JbSeekName);
			setEmail(res.data[0].JbSeekEmail);
			setExperience(res.data[0].JbSeekExperience);
			setfield(res.data[0].JbSeekField);
			setuniversityName(res.data[0].JbSeekUniversityName);
		  })
		  .catch((err) => console.log(err, "it has an error"));
	  }, []);
	const saveData = async() =>
  { const formData = new FormData();
	formData.append('JbSeekName', name);
	formData.append('JbSeekEmail', email);
	formData.append('JbSeekField', field);
	formData.append('JbSeekExperience', Experience);
	formData.append('JbSeekUniversityName', universityName);
	formData.append('file', file);
	axios.post("http://localhost:8003/jobseeker", formData)
  .then(response => {
								 ...
								 <form onSubmit={ saveData}>
						 			<div class="row">
										 ...
										 <input className='imagesss'onChange={(e)=>{setfile(e.target.files[0])} }  type="file"required/>
					 						<span class="pf-title">Job Seeker Name</span>
					 						<div class="pf-field">
					 							<input placeholder="Name"  onChange={(e)=>{setname(e.target.value)}} value={name} type="text"required/>
					 						</div>
					 					</div>
					 					<div class="col-lg-6">
					 						<span class="pf-title">Job Seeker Email</span>
					 						<div class="pf-field">
					 							<input placeholder="Email" type="text"  onChange={(e)=>{setEmail(e.target.value)}} value={email} class="form-control datepicker"required/>
					 						</div>
					 					</div>
					 					<div class="col-lg-6">
					 						<span class="pf-title">Job Seeker Field</span>
					 						<div class="pf-field">
					 							<input placeholder="Field" type="text"  onChange={(e)=>{setfield(e.target.value)}} value={field} class="form-control datepicker"required/>
					 						</div>
					 					</div>
					 					<div class="col-lg-12">
					 						<span class="pf-title">Job Seeker Skills</span>
					 						<div class="pf-field">
					 							<input placeholder="Skills" onChange={(e)=>{setuniversityName(e.target.value)}} value={universityName} type="text"required/>
					 						</div>
					 					</div>
					 					<div class="col-lg-12">
					 						<span class="pf-title">Job Seeker Experience</span>
					 						<div class="pf-field">
											 <input placeholder="Experience"  onChange={(e)=>{setExperience(e.target.value)} } value={Experience} type="text"required/>
					 						</div>
					 					</div>
					 					<div class="col-lg-12">
